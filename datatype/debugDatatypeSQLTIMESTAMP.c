...
void randTimestamp(TIMESTAMP_STRUCT *timestamp)
{
	timestamp->year = 2000 + (SQLSMALLINT)((99 * 1.0) * rand()/(RAND_MAX + 1.0));
    timestamp->month = 1 + (SQLUSMALLINT)((12 * 1.0) * rand()/(RAND_MAX + 1.0));
    timestamp->day = 1 + (SQLUSMALLINT)((28 * 1.0) * rand()/(RAND_MAX + 1.0));
	timestamp->hour=(SQLUSMALLINT)((23 * 1.0) * rand()/(RAND_MAX + 1.0));
    timestamp->minute = (SQLUSMALLINT)((59 * 1.0) * rand()/(RAND_MAX + 1.0));
    timestamp->second = (SQLUSMALLINT)((59 * 1.0) * rand()/(RAND_MAX + 1.0));
	timestamp->fraction = 0;
}

int SQLBindParameter_SQL_C_CHAR_to_SQL_TIMESTAMP(t_ODBCHDLInfo *pTestInfo)
{
	...
	LogMsg(NONE, "\nstart  test %s..............\n", __FUNCTION__);
	char tmpIn[200] = "2083-05-22 18:53:11";
	char tmpOut[200] = {0};
	
	SQLExecDirect(hstmt, (SQLTCHAR*)"DROP TABLE IF EXISTS TB_TIMESTAMP", SQL_NTS);
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"CREATE TABLE TB_TIMESTAMP (C TIMESTAMP)", SQL_NTS);
	...
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_TYPE_TIMESTAMP, sizeof(tmpIn), 0, tmpIn, 0, &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"INSERT INTO TB_TIMESTAMP VALUES(?)", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLExecDirect for insert fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"SELECT C FROM TB_TIMESTAMP", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
...
int SQLBindParameter_SQL_C_WCHAR_to_SQL_TIMESTAMP(t_ODBCHDLInfo *pTestInfo)
{
	SQLLEN StrLen_or_IndPtr;
	RETCODE retcode;
	SQLHANDLE 	henv;
	...
	LogMsg(NONE, "\nstart  test %s..............\n", __FUNCTION__);
	char tmpIn[200] = {0x32, 0x00, 0x30, 0x00, 0x38, 0x00, 0x33, 0x00, 0x2D, 0x00, 0x30, 0x00, 0x35, 0x00, 0x2D, 0x00, 0x32, 0x00, 0x32, 0x00, 0x20, 0x00, 0x31, 0x00, 0x38, 0x00, 0x3A, 0x00, 0x35, 0x00, 0x33, 0x00, 0x3A, 0x00, 0x31, 0x00, 0x31, 0x00};
	char tmpOut[200] = {0};
	...
	StrLen_or_IndPtr = SQL_NTS;
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_WCHAR, SQL_TYPE_TIMESTAMP, 0, 0, tmpIn, 19, &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
...
//#ifdef __TRAFODION__ 
	...
	/*****************test********************/
	SQLBindParameter_SQL_C_CHAR_to_SQL_TIMESTAMP(&mODBCHDLInfo);
	SQLFreeStmt(mODBCHDLInfo.hstmt, SQL_CLOSE);
	SQLBindParameter_SQL_C_WCHAR_to_SQL_TIMESTAMP(&mODBCHDLInfo);
	/***************************************/
