...
int SQLBindParameter_SQL_C_UBIGINT_to_SQL_DECIMAL(t_ODBCHDLInfo *pTestInfo)
{
	...
	char dropStr[] = "DROP TABLE IF EXISTS TB_DECIMAL";
	char createStr[] = "CREATE TABLE TB_DECIMAL (ID INT, C DECIMAL(18,10))";
	char insertStr[] = "INSERT INTO TB_DECIMAL VALUES (?,?)";
	char selectStr[] = "SELECT C FROM TB_DECIMAL";
	...
	retcode = SQLBindParameter(hstmt, 2, SQL_PARAM_INPUT, SQL_C_UBIGINT, SQL_DECIMAL, 18, 18, &tmpUBigInt, sizeof(tmpUBigInt), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter 2 fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"INSERT INTO TB_DECIMAL VALUES (?,?)", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
	...
	sprintf(outBuf, "SELECT C FROM TB_DECIMAL WHERE ID=%d", id);
	LogMsg(NONE, "%s\n", outBuf);
...
int SQLBindParameter_SQL_C_NUMERIC_to_SQL_DECIMAL(t_ODBCHDLInfo *pTestInfo)
{
	SQLLEN StrLen_or_IndPtr;
	RETCODE retcode;
	SQLHANDLE 	henv;
	...
	henv = pTestInfo->henv;
 	...
 	hstmt = (SQLHANDLE)pTestInfo->hstmt;
	char dropStr[] = "DROP TABLE IF EXISTS TB_DECIMAL";
	char createStr[] = "CREATE TABLE TB_DECIMAL (C DECIMAL(18,10))";
	char insertStr[] = "INSERT INTO TB_DECIMAL VALUES (12.233)";
	char selectStr[] = "SELECT C FROM TB_DECIMAL";
...
//#ifdef __TRAFODION__ 
	...
	/*****************test********************/
	SQLBindParameter_SQL_C_UBIGINT_to_SQL_DECIMAL(&mODBCHDLInfo);
	SQLFreeStmt(mODBCHDLInfo.hstmt, SQL_CLOSE);
	SQLBindParameter_SQL_C_NUMERIC_to_SQL_DECIMAL(&mODBCHDLInfo);
	/***************************************/
