...
int SQLBindParameter_SQL_C_CHAR_to_SQL_DATE(t_ODBCHDLInfo *pTestInfo)
{
	...
	LogMsg(NONE, "\nstart  test %s..............\n", __FUNCTION__);
	char tmpIn[200] = "2017-01-01";
	char tmpOut[200] = {0};
	
	SQLExecDirect(hstmt, (SQLTCHAR*)"DROP TABLE IF EXISTS TB_DATE", SQL_NTS);
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"CREATE TABLE TB_DATE (C DATE)", SQL_NTS);
	...
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_TYPE_DATE, sizeof(DATE_STRUCT), 0, tmpIn, strlen(tmpIn), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"INSERT INTO TB_DATE VALUES(?)", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLExecDirect for insert fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"SELECT C FROM TB_DATE", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
...
int SQLBindParameter_SQL_C_WCHAR_to_SQL_DATE(t_ODBCHDLInfo *pTestInfo)
{
	SQLLEN StrLen_or_IndPtr;
	RETCODE retcode;
	SQLHANDLE 	henv;
	...
	LogMsg(NONE, "\nstart  test %s..............\n", __FUNCTION__);
	//"2083-05-22"
	char tmpIn[200] = {0x32, 0x00,0x30, 0x00,0x38, 0x00,0x33, 0x00,0x2D, 0x00,0x30, 0x00,0x35, 0x00,0x2D, 0x00,0x32, 0x00,0x32, 0x00};
	char tmpOut[200] = {0};
	...
	StrLen_or_IndPtr = SQL_NTS;
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_WCHAR, SQL_TYPE_DATE, sizeof(tmpIn), 0, tmpIn, 10, &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
...
//#ifdef __TRAFODION__ 
	...
	/*****************test********************/
	SQLBindParameter_SQL_C_CHAR_to_SQL_DATE(&mODBCHDLInfo);
	SQLFreeStmt(mODBCHDLInfo.hstmt, SQL_CLOSE);
	SQLBindParameter_SQL_C_WCHAR_to_SQL_DATE(&mODBCHDLInfo);
	/***************************************/
