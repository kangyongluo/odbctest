...
int SQLBindParameter_SQL_C_CHAR_to_SQL_TIME(t_ODBCHDLInfo *pTestInfo)
{
	...
	LogMsg(NONE, "\nstart  test %s..............\n", __FUNCTION__);
		//#ifdef __TRAFODION__
#if 0
	char tmpIn[200] = "TIME '12:12:00'";
#else
	char tmpIn[200] = "12:12:00";
#endif
	
	char tmpOut[200] = {0};
	
	SQLExecDirect(hstmt, (SQLTCHAR*)"DROP TABLE IF EXISTS TB_TIME", SQL_NTS);
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"CREATE TABLE TB_TIME (C TIME)", SQL_NTS);
	...
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_TYPE_TIME, sizeof(tmpIn), 0, tmpIn, strlen(tmpIn), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"INSERT INTO TB_TIME VALUES(?)", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLExecDirect for insert fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"SELECT C FROM TB_TIME", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
...
int SQLBindParameter_SQL_C_WCHAR_to_SQL_TIME(t_ODBCHDLInfo *pTestInfo)
{
	SQLLEN StrLen_or_IndPtr;
	RETCODE retcode;
	SQLHANDLE 	henv;
	...
	LogMsg(NONE, "\nstart  test %s..............\n", __FUNCTION__);
	char tmpIn[200] = {0x31, 0x00, 0x32, 0x00, 0x3a, 0x00, 0x31, 0x00, 0x32, 0x00, 0x3a, 0x00, 0x31, 0x00, 0x32, 0x00};
	char tmpOut[200] = {0};
	...
	StrLen_or_IndPtr = SQL_NTS;
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_WCHAR, SQL_TYPE_TIME, sizeof(tmpIn), 0, tmpIn, 8, &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
...
//#ifdef __TRAFODION__ 
	...
	/*****************test********************/
	SQLBindParameter_SQL_C_CHAR_to_SQL_TIME(&mODBCHDLInfo);
	SQLFreeStmt(mODBCHDLInfo.hstmt, SQL_CLOSE);
	SQLBindParameter_SQL_C_WCHAR_to_SQL_TIME(&mODBCHDLInfo);
	/***************************************/
