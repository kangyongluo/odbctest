/*
*测试目的:测试动态绑定参数
*				绑定的C数据类型为SQL_C_WCHAR，ODBC数据类型为SQL_VARCHAR
*
*测试结果:
*			esgynDB:
*			如SQL_C_WCHAR数据内容为0x31 ,0x00, 0x32, 0x00, 0x33, 0x00, 0x38, 0x00,只能成功插入第一个字节
*
*			MySQL:成功执行;
*/
int SQLBindParameterSQLCWcharToVarchar(SQLHANDLE hstmt)
{
	SQLLEN len;
	RETCODE retcode;
	unsigned char tmpw[10] = {0x31 ,0x00, 0x32, 0x00, 0x33, 0x00, 0x38, 0x00};
	
	retcode = SQLExecDirect(hstmt, (SQLCHAR*)"drop table datatype", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		printf("drop table fail:\n");
	}
	retcode = SQLExecDirect(hstmt, (SQLCHAR*)"create table datatype(c VARCHAR(30))", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
...
*						------------------------------
*						123
			读出来的数据为:0x31 0x32 0x33
*			MySQL:读出来的数据为:0x31 0x00 0x32 0x00 0x33 0x00
*/

int SQLBindColSQLWcharToVarchar(SQLHANDLE hstmt)
{
	SQLLEN len;
	RETCODE retcode;
	...
	retcode = SQLExecDirect(hstmt, (SQLCHAR*)"create table datatype(c VARCHAR(20))", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		printf("create table fail:\n");
		return -1;
	}
...
/*
*测试目的:测试绑定参数写SQL
*				绑定的C数据类型为SQL_C_UBIGINT，ODBC数据类型为SQL_VARCHAR
*
*测试结果:
...
*			MySQL:成功执行;
*/
int SQLBindParameterSQLCUbigintToSQLVarchar(SQLHANDLE hstmt)
{
	SQLLEN len;
	RETCODE retcode;
...
int SQLBindColSQLUbigintToVarchar(SQLHANDLE hstmt)
{
	SQLLEN len;
	RETCODE retcode;
...
/*
*测试目的:测试动态绑定参数
*				绑定的C数据类型为SQL_C_NUMERIC，ODBC数据类型为SQL_VARCHAR
*
*测试结果:
...
int SQLBindParameterSQLNumericToSQLVARChar(SQLHANDLE hstmt)
{
	SQLLEN len;
	RETCODE retcode;
...
/*
*测试目的:测试动态绑定参数
*				绑定的C数据类型为SQL_C_BINARY，ODBC数据类型为SQL_VARCHAR
*
*测试结果:
*			esgynDB:绑定后执行SQLExecDirect(hstmt, "insert into table values(?)", SQL_NTS)失败
*			MySQL:成功执行;
...
int SQLBindParameterSQLCBinaryToVarchar(SQLHANDLE hstmt)
{
	SQLLEN len;
	RETCODE retcode;
	unsigned char data[20] = "8.548180e+01";
