...
int SQLBindParameter_SQL_C_NUMERIC_to_SQL_BIGINT(t_ODBCHDLInfo *pTestInfo)
{
	...
	SQLExecDirect(hstmt, (SQLTCHAR*)"DROP TABLE IF EXISTS TB_BIGINT", SQL_NTS);
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"CREATE TABLE TB_BIGINT (C LARGEINT)", SQL_NTS);
	...
	LogMsg(NONE, "\n");
	StrLen_or_IndPtr = SQL_NTS;
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_NUMERIC, SQL_BIGINT, 0, 0, &numeric, sizeof(numeric), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"INSERT INTO TB_BIGINT VALUES (?)", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLExecDirect for insert fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"SELECT C FROM TB_BIGINT", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
...
int SQLBindParameter_SQL_C_CHAR_to_SQL_BIGINT(t_ODBCHDLInfo *pTestInfo)
{
	SQLLEN StrLen_or_IndPtr;
	RETCODE retcode;
	SQLHANDLE 	henv;
	...
	LogMsg(NONE, "\nstart  test %s..............\n", __FUNCTION__);
	char bufIn[200] = "-9223372036854775808";
	char bufOut[200] = {0};
	...
	StrLen_or_IndPtr = SQL_NTS;
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_BIGINT, sizeof(bufIn), 0, bufIn, sizeof(bufIn), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	...
	retcode = SQLBindCol(hstmt, 1, SQL_C_CHAR, bufOut, sizeof(bufOut), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindCol fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
...
int SQLBindParameter_SQL_C_CHAR_to_SQL_UBIGINT(t_ODBCHDLInfo *pTestInfo)
{
	SQLLEN StrLen_or_IndPtr;
	RETCODE retcode;
	SQLHANDLE 	henv;
	...
	LogMsg(NONE, "\nstart  test %s..............\n", __FUNCTION__);
	char bufIn[200] = "9223372036854775807";
	char bufOut[200] = {0};
	
	SQLExecDirect(hstmt, (SQLTCHAR*)"DROP TABLE IF EXISTS TB_BIGINT", SQL_NTS);
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"CREATE TABLE TB_BIGINT (C LARGEINT UNSIGNED)", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "create table fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
...
int SQLBindParameter_SQL_C_NUMERIC_to_SQL_UBIGINT(t_ODBCHDLInfo *pTestInfo)
{
	SQLLEN StrLen_or_IndPtr;
	RETCODE retcode;
	SQLHANDLE 	henv;
...
//#ifdef __TRAFODION__ 
	...
	/*****************test********************/
	SQLBindParameter_SQL_C_NUMERIC_to_SQL_BIGINT(&mODBCHDLInfo);
	SQLFreeStmt(mODBCHDLInfo.hstmt, SQL_CLOSE);
	SQLBindParameter_SQL_C_CHAR_to_SQL_BIGINT(&mODBCHDLInfo);
	SQLFreeStmt(mODBCHDLInfo.hstmt, SQL_CLOSE);
	SQLBindParameter_SQL_C_CHAR_to_SQL_UBIGINT(&mODBCHDLInfo);
	SQLFreeStmt(mODBCHDLInfo.hstmt, SQL_CLOSE);
	SQLBindParameter_SQL_C_NUMERIC_to_SQL_UBIGINT(&mODBCHDLInfo);
	/***************************************/
