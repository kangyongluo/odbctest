...
int SQLBindParameter_SQL_C_CHAR_to_SQL_REAL(t_ODBCHDLInfo *pTestInfo)
{
	...
	SQLExecDirect(hstmt, (SQLTCHAR*)"DROP TABLE IF EXISTS TB_REAL", SQL_NTS);
	LogMsg(NONE, "%s\n", createStr);
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"CREATE TABLE TB_REAL (C REAL)", SQL_NTS);
	...
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_DOUBLE, SQL_REAL, 0, 0, &tmpIn, sizeof(tmpIn), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"INSERT INTO TB_REAL VALUES (?)", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLExecDirect for insert fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"SELECT C FROM TB_REAL", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLExecDirect for select fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLBindCol(hstmt, 1, SQL_C_DOUBLE, &tmpOut, sizeof(tmpOut), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
	...
	LogMsg(NONE, "expect:%lg\tactual:%lg\n", tmpIn, tmpOut);
	return 0;
}
int main(void)
{
...
//#ifdef __TRAFODION__ 
	...
	/*****************test********************/
	SQLBindParameter_SQL_C_CHAR_to_SQL_REAL(&mODBCHDLInfo);
	/***************************************/
