...
int SQLBindParameter_SQL_C_CHAR_to_SQL_FLOAT(t_ODBCHDLInfo *pTestInfo)
{
	...
	LogMsg(NONE, "\nstart  test %s..............\n", __FUNCTION__);
	char tmpIn[200] = "1.7549e-38";
	char tmpOut[200] = {0};
	/************************************************************/
	//The result is correct in this way
	float fa = 1.7549e-38;
	float fout = 0.0;
	SQLExecDirect(hstmt, (SQLTCHAR*)"DROP TABLE IF EXISTS TB_FLOAT", SQL_NTS);
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"CREATE TABLE TB_FLOAT (C FLOAT)", SQL_NTS);
	...
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_FLOAT, SQL_FLOAT, 0, 0, &fa, sizeof(fa), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"INSERT INTO TB_FLOAT VALUES (?)", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLExecDirect for insert fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
	}
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"SELECT C FROM TB_FLOAT", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
	...
	retcode = SQLBindCol(hstmt, 1, SQL_C_FLOAT, &fout, sizeof(fout), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
	...
	LogMsg(NONE, "expect:%le\tactual:%le\n", fa, fout);
	...
	*/
	...
	StrLen_or_IndPtr = SQL_NTS;
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_FLOAT, strlen(tmpIn), 0, tmpIn, strlen(tmpIn), &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
...
int SQLBindParameter_SQL_C_WCHAR_to_SQL_FLOAT(t_ODBCHDLInfo *pTestInfo)
{
	SQLLEN StrLen_or_IndPtr;
	RETCODE retcode;
	SQLHANDLE 	henv;
	...
	SQLExecDirect(hstmt, (SQLTCHAR*)"DROP TABLE IF EXISTS TB_FLOAT", SQL_NTS);
	//retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"CREATE TABLE TB_FLOAT (C FLOAT(54))", SQL_NTS);
	retcode = SQLExecDirect(hstmt, (SQLTCHAR*)"CREATE TABLE TB_FLOAT (C FLOAT)", SQL_NTS);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "create table fail:\n");
		LogAllErrors(henv,hdbc,hstmt);
	...
	tmpIn[2] = 0x32;
	tmpIn[3] = 0x00;
	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_WCHAR, SQL_FLOAT, 2, 0, tmpIn, 2, &StrLen_or_IndPtr);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, "SQLBindParameter fail\n");
		LogAllErrors(henv,hdbc,hstmt);
		return -1;
...
//#ifdef __TRAFODION__ 
	...
	/*****************test********************/
	SQLBindParameter_SQL_C_CHAR_to_SQL_FLOAT(&mODBCHDLInfo);
	SQLFreeStmt(mODBCHDLInfo.hstmt, SQL_CLOSE);
	SQLBindParameter_SQL_C_WCHAR_to_SQL_FLOAT(&mODBCHDLInfo);
	/***************************************/
