...
int CConfig::scanTestTable(void)
{
    ...
    pODBCTestData->isAvailableLoad = FALSE;
    pODBCTestData->isAvailableTable = FALSE;
    pODBCTestData->isAvailableLob = FALSE;
    if(isTableDefault){
        if(mFd != 0){
            return -1;
        }
    ...
    if(ret == 0){
        ...
        if(readLoadInfo(szSection, &pODBCTestData->mLoadDataInfo) == 0)
            pODBCTestData->isAvailableLoad = TRUE;
        if(readRowsetInfo(szSection, &pODBCTestData->mRowsetInfo) == 0)
                pODBCTestData->isAvailableRowset = TRUE;
        if(readLobInfo(szSection, &pODBCTestData->mLobInfo) == 0)
                pODBCTestData->isAvailableLob = TRUE;
        mFd++;
        pODBCTestData->isAvailableTable = TRUE;
        return 0;
    }
...
int CConfig::readLoadInfo(char *pSection, sLoadDataInfo *psLoadDataInfo)
{
    char szBuf[256] = {0};
    char *result;
    int i;
...
int CConfig::readLobInfo(char *pSection, sLoadDataInfo *psLoadDataInfo)
{
    int i;
    char szBuf[256] = {0};
    char *result;
...
int CConfig::readSqlFile(char *pSection, sSqlStmt *psSqlFileInfo)
{
    ...
    if(ret == 0){
        ...
        for(j = 0; j < CONFIG_TEST_RESULT_ROWS; j++){
            _stprintf(szKey, _T("%d"), _T("result"), (j == 0) ? "" : "_", j);
            getConfigString(pSection, 
                            szKey, 
                            "", 
                            szBuf, 
