...
void CConfig::setDefaultTable(void)
{
    ...
    pODBCTestData->mTableInfo.isBlobTable = TRUE;
    pODBCTestData->mTableInfo.numBlobCols = 1;
    pODBCTestData->mTableInfo.idBlobCols[0] = 0;
...
int CConfig::readTable(char *pSection, sTestTableInfo *psTestTableInfo)
{
    ...
    result = strtok(szBuf, ",");
    while( result != NULL ) {
        psTestTableInfo->dataWidth[j++][0] = atoi(result);
        result = strtok(NULL, ",");
        if(j >= CONFIG_SQL_COLUMNS_MAX) break;
    }
    memset(szBuf, 0, sizeof(szBuf));
    ...
    result = strtok(szBuf, ",");
    while( result != NULL ) {
        psTestTableInfo->dataWidth[j++][1] = atoi(result);
        result = strtok(NULL, ",");
        if(j >= CONFIG_SQL_COLUMNS_MAX) break;
    }
    memset(szBuf, 0, sizeof(szBuf));
    ...
    for(i = 0; i < psTestTableInfo->columNum; i++){
        if(psTestTableInfo->dataWidth[i][0] > psTestTableInfo->columnSize[i]){
            psTestTableInfo->dataWidth[i][0] = psTestTableInfo->columnSize[i];
        }
        if(psTestTableInfo->dataWidth[i][1] > psTestTableInfo->decimalDigits[i]){
            psTestTableInfo->dataWidth[i][1] = psTestTableInfo->decimalDigits[i];
        }
    }
    getConfigBool(pSection, 
                    "lob_table", 
                    "FLASE", 
                    &psTestTableInfo->isBlobTable, 
                    ODBC_TEST_CFG_FILE);
    getConfigString(pSection, 
                    ...
                    sizeof(szBuf), 
                    ODBC_TEST_CFG_FILE);
    psTestTableInfo->numBlobCols = 0;
    if(strlen(szBuf) > 0){
        result = strtok(szBuf, ",");
        while( result != NULL ) {
            psTestTableInfo->idBlobCols[psTestTableInfo->numBlobCols++] = atoi(result);
            result = strtok(NULL, ",");
            if(psTestTableInfo->numBlobCols >= (sizeof(psTestTableInfo->idBlobCols) / sizeof(psTestTableInfo->idBlobCols[0]))) break;
        }
    }
...
int CConfig::scanTestTable(void)
{
    ...
    pODBCTestData->isAvailableBindPara = FALSE;
    pODBCTestData->isAvailableTable = FALSE;
    pODBCTestData->isAvailablePutData = FALSE;
    if(isTableDefault){
        if(mFd != 0){
            return -1;
        }
    ...
    if(ret == 0){
        ...
        if(readSQLBindParaInfo(szSection, &pODBCTestData->mBindParaInfo) == 0)
            pODBCTestData->isAvailableBindPara = TRUE;
        if(readRowsetInfo(szSection, &pODBCTestData->mRowsetInfo) == 0)
                pODBCTestData->isAvailableRowset = TRUE;
        if(readSQLPutDataInfo(szSection, &pODBCTestData->mPutDataInfo) == 0)
                pODBCTestData->isAvailablePutData = TRUE;
        mFd++;
        pODBCTestData->isAvailableTable = TRUE;
        return 0;
    }
...
int CConfig::readSQLBindParaInfo(char *pSection, sLoadDataInfo *psLoadDataInfo)
{
    char szBuf[256] = {0};
    char *result;
    int i;
    ...
    getConfigString(pSection, 
                    "SQLBindParameter_file", 
                    "", 
                    psLoadDataInfo->szBlobFile, 
                    sizeof(psLoadDataInfo->szBlobFile), 
                    ODBC_TEST_CFG_FILE);
    if(_tcslen(psLoadDataInfo->szBlobFile) > 0){
        psLoadDataInfo->putDataType = SQL_DATA_AT_EXEC;
    }
    getConfigBool(pSection, 
                    "SQLBindParameter_file_crc", 
                    "FLASE", 
...
int CConfig::readSQLPutDataInfo(char *pSection, sLoadDataInfo *psLoadDataInfo)
{
    int i;
    char szBuf[256] = {0};
    char *result;
    ...
    getConfigInt(pSection, 
                "SQLPutData_batch_size", 
                "2", 
                &psLoadDataInfo->batchSize,
                ODBC_TEST_CFG_FILE);
    getConfigInt(pSection, 
                "SQLPutData_batch_num", 
                "1", 
                &psLoadDataInfo->totalBatch,
                ODBC_TEST_CFG_FILE);
    getConfigInt(pSection, 
                "SQLPutData_begin_value", 
                "0", 
                &psLoadDataInfo->beginVal,
                ODBC_TEST_CFG_FILE);
    getConfigString(pSection, 
                    "SQLPutData_load_type", 
                    "INSERT", 
                    psLoadDataInfo->szInsertType, 
                    sizeof(psLoadDataInfo->szInsertType), 
                    ODBC_TEST_CFG_FILE);
