...
/*******************************************************************************************************************/
int debugSQLTables(TestInfo *pTestInfo)
{
    int ret = 0;
    RETCODE retcode;
 	SQLHANDLE henv = SQL_NULL_HANDLE;
    ...
    //retcode = SQLTables(hstmt, NULL, 0, (SQLTCHAR*)SQL_ALL_SCHEMAS, SQL_NTS, NULL, 0, NULL, 0);
    //retcode = SQLTables(hstmt, (SQLTCHAR *)"TRAFODION", SQL_NTS, (SQLTCHAR*)"%", SQL_NTS, (SQLTCHAR *)"%", SQL_NTS, (SQLTCHAR *)"%", SQL_NTS);
    //retcode = SQLTables(hstmt, (SQLTCHAR *)"%", SQL_NTS, (SQLTCHAR*)"%", SQL_NTS, (SQLTCHAR *)"%", SQL_NTS, (SQLTCHAR *)"%", SQL_NTS);
    retcode = SQLTables(hstmt, (SQLTCHAR*)"TRAFODION", SQL_NTS, (SQLTCHAR*)"ODBCTEST", SQL_NTS, (SQLTCHAR*)"TB", SQL_NTS, (SQLTCHAR*)"", SQL_NTS);
    //if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
    ...
    while (retcode != SQL_NO_DATA){
        for (i = 1; i <= num; ++i){
            ...
            retcode = SQLGetData(hstmt, i, SQL_C_TCHAR, (SQLPOINTER)buf, sizeof(buf), &len);
            if (SQL_SUCCEEDED(retcode)){
                printf("%s\t", buf);
            }
            else{
