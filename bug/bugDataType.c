...
/*******************************************************************************************************************/
int debugSQLCFloatSQLNumeric(TestInfo *pTestInfo)
{
    int ret = 0;
    RETCODE retcode;
 	SQLHANDLE henv = SQL_NULL_HANDLE;
    ...
    LogMsg(NONE, _T("..............................\n"));
 #if 1   
    /*LogMsg(NONE, _T("create table tb(c1 numeric(18,0))\n"));
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("drop table tb"), SQL_NTS);
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("create table tb(C1 NUMERIC(18,0), C2 NUMERIC(18,18), C3 NUMERIC(18,17), C4 NUMERIC(19,0), C5 NUMERIC(19,19), C6 NUMERIC(19,18), C7 NUMERIC(128,0), C8 NUMERIC(64), C9 NUMERIC(128,128), C10 NUMERIC(128,127), C11 NUMERIC(18), C12 NUMERIC(19,10), C13 NUMERIC (128), C14 NUMERIC(64,32)) NO PARTITION"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("create table fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    }*/
    LogMsg(NONE, _T("insert into tb values(12345678, 0.12345678, 0.12345678, 12345678, 0.12345678, 0.12345678, 12345678, 12345678, 0.12345678, 0.12345678, 12345678, 12345678, 12345678, 0.12345678)\n"));
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("insert into tb values(12345678, 0.12345678, 0.12345678, 12345678, 0.12345678, 0.12345678, 12345678, 12345678, 0.12345678, 0.12345678, 12345678, 12345678, 12345678, 0.12345678)"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("insert table fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        LogAllErrors(henv,hdbc,hstmt);
        goto TEST_FAIL;
    }
#else
    /*LogMsg(NONE, _T("purgedata tb\n"));
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("purgedata tb"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("purgedata tb fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    }*/
#endif
...
#endif
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("select c1 from tb"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("insert fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    }
    retcode = SQLFetch(hstmt);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("fetch fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    }
    ...
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("SQLGetData fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    }
    LogMsg(NONE, "expect:%s actual:%s\n", "12345678", szBuf);
    if(strncasecmp(szBuf, "12345678", 8) != 0){
        ret = -1;
    }
TEST_FAIL:
...
int main(int argc, char *argv[])
{
    ...
    *R2.5 :
    *10.10.14.4
    *daily20181019_05_rh6
    */
    debugSQLCFloatSQLNumeric(&oriInfo);
    LogMsg(NONE, _T("\n\n"));
#ifndef unixcli
    printf("Please enter any key exit\n");
    getchar();
