...
/*******************************************************************************************************************/
int debugSQLSetStmtAttr(TestInfo *pTestInfo)
{
    int ret = 0;
    RETCODE retcode;
 	SQLHANDLE henv = SQL_NULL_HANDLE;
    ...
    LogMsg(NONE, _T("..............................\n"));
    valueAttr = 10;
    retcode = SQLSetStmtAttr(hstmt, SQL_MAX_LENGTH, (void *)valueAttr, SQL_IS_UINTEGER);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("call SQLSetStmtAttr fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        LogAllErrors(henv,hdbc,hstmt);
        ret = -1;
        goto TEST_FAIL;
    }
#if 1
    LogMsg(NONE, _T("create table tb(c1 char(100) CHARACTER SET ISO88591, c2 varchar(1000) CHARACTER SET ISO88591)\n"));
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("drop table tb"), SQL_NTS);
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("create table tb(c1 char(100) CHARACTER SET ISO88591, c2 varchar(1000) CHARACTER SET ISO88591)"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
    ...
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("insert into tb values('0123456789','abcde12345')"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("inert fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        LogAllErrors(henv,hdbc,hstmt);
        ret = -1;
        goto TEST_FAIL;
    }
    ...
    if(retcode != SQL_SUCCESS){
        LogAllErrors(henv,hdbc,hstmt);
        LogMsg(NONE, _T("select fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    }
    ...
    while (retcode != SQL_NO_DATA)
    {
        for (i = 1; i <= num; ++i)
        {
            retcode = SQLGetData(hstmt, i, SQL_C_CHAR, (SQLPOINTER)buf, sizeof(buf), &len);
            if (SQL_SUCCEEDED(retcode)){
                LogMsg(NONE, _T("row:%d column:%d value:%s\n"), rows, i, buf);
            }
            else{
                break;
            }
...
int SQLGetTypeInfoAllType(TestInfo *pTestInfo)
{
    RETCODE            retcode;
    SQLHANDLE 	henv = SQL_NULL_HANDLE;
 	SQLHANDLE 	hdbc = SQL_NULL_HANDLE;
...
int debugSQLDescribeCol(TestInfo *pTestInfo)
{
    ...
    LogMsg(NONE, _T("..............................\n"));
#if 1
    LogMsg(NONE, _T("create table tb(c1 date, c2 time, c3 timestamp)\n"));
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("drop table tb"), SQL_NTS);
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("create table tb(c1 date, c2 time, c3 timestamp)"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("create table fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        LogAllErrors(henv,hdbc,hstmt);
        ret = -1;
    ...
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("insert into tb values('2018-10-30', '12:12:12', '2018-10-30 12:12:12')"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("inert fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        LogAllErrors(henv,hdbc,hstmt);
        ret = -1;
    ...
    while (retcode != SQL_NO_DATA){
        for (i = 1; i <= num; ++i){
            retcode = SQLDescribeCol (hstmt,
                                  ...
                                  &decimaldigits,
                                  &nullable);
            LogMsg(NONE, _T("columns:%d colName:%s colLen:%d type:%d\n"), i, colname, colnamelen, coltype);
            switch(i){
            case 1:
                if(coltype != SQL_TYPE_DATE){
                    ret = -1;
                    LogMsg(NONE, _T("sql type are not match:expect:%d actual:%d\n"), SQL_TYPE_DATE, coltype);
                }
                break;
            case 2:
                if(coltype != SQL_TYPE_TIME){
                    ret = -1;
                    LogMsg(NONE, _T("sql type are not match:expect:%d actual:%d\n"), SQL_TYPE_TIME, coltype);
                }
                break;
            case 3:
                if(coltype != SQL_TYPE_TIMESTAMP){
                    ret = -1;
                    LogMsg(NONE, _T("sql type are not match:expect:%d actual:%d\n"), SQL_TYPE_TIMESTAMP, coltype);
                }
                break;
            }
        }
...
int debugSQLFreeStmt(TestInfo *pTestInfo)
{
    int ret = 0;
    RETCODE retcode;
 	SQLHANDLE henv = SQL_NULL_HANDLE;
 	SQLHANDLE hdbc = SQL_NULL_HANDLE;
 	SQLHANDLE hstmt = SQL_NULL_HANDLE;
    SQLHANDLE hdbc1 = SQL_NULL_HANDLE;
    TestInfo sTestInfo;
    SQLSMALLINT num, i;
    char szBuf[512] = {0};
    SQLLEN len;
    ...
    addInfoSession(hstmt);
    LogMsg(NONE, _T("..............................\n"));
    //If you close this part of code and call SQLFreeStmt (hstmt, SQL_DROP), there will be no problem.
#if 1 
    retcode = SQLAllocConnect((SQLHANDLE)henv,(SQLHANDLE *)&hdbc1);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
        LogMsg(NONE, _T("call SQLSetConnectOption fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    }

	retcode = SQLSetConnectOption((SQLHANDLE)hdbc1, SQL_LOGIN_TIMEOUT, timeout);
	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO)){
		LogMsg(NONE, _T("call SQLSetConnectOption fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    ...
    retcode = SQLConnect(hdbc1,
                           (SQLTCHAR*)sTestInfo.DataSource,(SWORD)_tcslen(sTestInfo.DataSource),
                           (SQLTCHAR*)sTestInfo.UserID,(SWORD)_tcslen(sTestInfo.UserID),
                           (SQLTCHAR*)sTestInfo.Password,(SWORD)_tcslen(sTestInfo.Password)
                           );

	if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO))
	{
		SQLFreeConnect(hdbc1);
        LogMsg(NONE, _T("call SQLConnect fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
	}
    SQLDisconnect((SQLHANDLE)hdbc1);
    SQLFreeConnect((SQLHANDLE)hdbc1);
#endif
    LogMsg(NONE, _T("SQLFreeStmt(hstmt,SQL_DROP)\n"));
    retcode = SQLFreeStmt(hstmt,SQL_DROP);
    if ((retcode != SQL_SUCCESS) && (retcode != SQL_SUCCESS_WITH_INFO))
	{
		LogMsg(NONE, _T("call SQLFreeStmt(hstmt, SQL_DROP) fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        LogAllErrors(henv, hdbc, hstmt);
        ret = -1;
	}
TEST_FAIL:
...
int main(int argc, char *argv[])
{
    ...
    //debugSQLFreeStmt(&oriInfo);
    //LogMsg(NONE, _T("\n\n"));
    debugSQLDescribeCol(&oriInfo);
    //LogMsg(NONE, _T("\n\n"));
    //debugSQLSetStmtAttr(&oriInfo);
    //SQLGetTypeInfoAllType(&oriInfo);
#ifndef unixcli
    printf("Please enter any key exit\n");
    getchar();
#endif
