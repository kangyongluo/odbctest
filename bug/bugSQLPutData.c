...
/*******************************************************************************************************************/
int debugSQLPutDataNull(TestInfo *pTestInfo)
{
    int ret = 0;
    RETCODE retcode;
 	SQLHANDLE henv = SQL_NULL_HANDLE;
    ...
    LogMsg(NONE, _T("..............................\n"));
    LogMsg(NONE, _T("create table tb(c1 varchar(100))\n"));
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("drop table tb"), SQL_NTS);
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("create table tb(c1 varchar(100))"), SQL_NTS);
    ...
    //retcode = SQLPutData(hstmt, NULL, SQL_NULL_DATA);
    retcode = SQLPutData(hstmt, "hello word", SQL_NTS);
    if(retcode != SQL_SUCCESS){
    ...
    retcode = SQLParamData(hstmt,&pToken);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("SQL_SUCCESS fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    }
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("select * from tb"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("call select fail.\n"));
        LogAllErrors(henv,hdbc,hstmt);
        ret = -1;
        goto TEST_FAIL;
    }
    retcode= SQLBindCol(hstmt, 1, SQL_C_TCHAR, buf, sizeof(buf), &len);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("call SQLBindCol fail.\n"));
        LogAllErrors(henv,hdbc,hstmt);
        ret = -1;
        goto TEST_FAIL;
    }
    ...
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("call SQLFetch fail.\n"));
        LogAllErrors(henv,hdbc,hstmt);
        ret = -1;
        goto TEST_FAIL;
    }
    LogMsg(NONE, _T("len:%d data:%s\n"), len, buf);
TEST_FAIL:
...
int debugBind(TestInfo *pTestInfo)
{
    int ret = 0;
    RETCODE retcode;
 	SQLHANDLE henv = SQL_NULL_HANDLE;
