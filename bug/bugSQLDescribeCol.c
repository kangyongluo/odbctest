...
int debugSQLDescribeCol(TestInfo *pTestInfo)
{
    ...
    LogMsg(NONE, _T("..............................\n"));
#if 0
    LogMsg(NONE, _T("create table tb(c1 int, c02 char(10))\n"));
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("drop table tb"), SQL_NTS);
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("create table tb(c1 int, c02 char(10))"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
        LogMsg(NONE, _T("create table fail ! %s line = %d\n"), __FUNCTION__, __LINE__);
        ret = -1;
        goto TEST_FAIL;
    }
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("insert into tb values(1, '123aa')"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
    ...
    retcode = SQLExecDirect(hstmt, (SQLTCHAR *)_T("call char_select()"), SQL_NTS);
    if(retcode != SQL_SUCCESS){
    ...
    while (retcode != SQL_NO_DATA){
        for (i = 1; i <= num; ++i){
            retcode = SQLDescribeCol (hstmt,
                                  ...
                                  &decimaldigits,
                                  &nullable);
            LogMsg(NONE, _T("columns:%d sqltype:%d colName:%s colLen:%d\n"), i, coltype, colname, colnamelen);
        }
        printf("\n");
        retcode = SQLFetch(hstmt);
    } 
